...
- `uv run index_game_code.py <source_root_path> <output_directory>` - Rebuild the code index
- `uv run hierarchy_tree.py` - Helper module for generating hierarchy tree text files (used by the indexer)
- `uv run copy_game_content.py` - Copy game content files (used by the preparation script)
