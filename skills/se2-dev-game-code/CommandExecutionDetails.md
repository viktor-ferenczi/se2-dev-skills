...
uv run search_code.py class declaration CubeGridComponent (with workdir set to the skill folder)
```
...
```cmd
C:\Users\name> uv run search_code.py class declaration CubeGridComponent
```
**Error:** `search_code.py` not found, wrong CWD

✅ **Solution 1:** Use workdir parameter:
```bash
uv run search_code.py class declaration CubeGridComponent (with workdir=C:\path\to\skill\folder)
```
...
```bash
uv run search_code.py class declaration CubeGridComponent (with workdir set to skill folder)
```
...
# Command 2
uv run search_code.py class declaration CubeGridComponent (workdir: skill_folder)
```

**Option B:** Use PowerShell with semicolons:
```powershell
cd C:\path\to\skill; .\Prepare.bat; uv run search_code.py class declaration CubeGridComponent
```
