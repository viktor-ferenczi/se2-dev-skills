...
This document provides comprehensive details about command execution on Windows for the se2-dev-server-code skill. For most use cases, refer to the Quick Start section in [CommandExecution.md](CommandExecution.md).
...
```cmd
cd C:\skills\se2-dev-server-code && Prepare.bat
```
...
```cmd
cd C:\skills\se2-dev-server-code
Prepare.bat
```
