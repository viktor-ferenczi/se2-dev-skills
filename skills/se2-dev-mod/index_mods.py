...
STEAM_MODS_DIR = SCRIPT_DIR / "SteamMods"
LOCAL_MODS_DIR = SCRIPT_DIR / "LocalMods"
OUTPUT_DIR = SCRIPT_DIR / "ModCodeIndex"
MOD_LIST_FILE = OUTPUT_DIR / "mods.json"
...
def find_steam_mods(steam_mods_dir: Path) -> List[Dict]:
    """Find mods in SteamMods directory (folders with Data/Scripts subdirectory)"""
    mods = []
    ...
    for item in steam_mods_dir.iterdir():
        if item.is_dir():
            ...
            if scripts_dir.exists() and any(scripts_dir.rglob("*.cs")):
                mods.append({
                    "id": item.name,
                    "name": item.name,
                    "source": "steam",
...
def find_local_mods(local_mods_dir: Path) -> List[Dict]:
    """Find mods in LocalMods directory"""
    mods = []
    if not local_mods_dir.exists():
        return mods

    for item in local_mods_dir.iterdir():
        if item.is_dir():
            # Check for any .cs files
...
class ModCodeIndexer:
    """Indexes mod C# source code using Tree-sitter with parallel processing"""
    ...
    def collect_files(self) -> Tuple[List[Path], List[Dict]]:
        """Collect all C# files from Steam and local mods"""
        cs_files = []
        all_mods = []

        # Find Steam mods
        steam_mods = find_steam_mods(STEAM_MODS_DIR)
        for mod in steam_mods:
            mod_path = SCRIPT_DIR / mod["path"]
            scripts_dir = mod_path / "Data" / "Scripts"
            if scripts_dir.exists():
                cs_files.extend(scripts_dir.rglob("*.cs"))
        all_mods.extend(steam_mods)

        # Find local mods
        local_mods = find_local_mods(LOCAL_MODS_DIR)
        for mod in local_mods:
            mod_path = SCRIPT_DIR / mod["path"]
            cs_files.extend(mod_path.rglob("*.cs"))
        all_mods.extend(local_mods)

        return cs_files, all_mods

    def index_files(self, cs_files: List[Path]):
        """Index all collected C# files using parallel processing"""
    ...
    def write_indices(self, output_dir: Path, mods: List[Dict]):
        ...
        print(f"Written mod list to {MOD_LIST_FILE}")
...
def main():
    # Increase recursion limit for deeply nested code
    sys.setrecursionlimit(10000)

    print("Mod Code Indexer")
    print("=" * 50)
    print(f"Steam mods directory: {STEAM_MODS_DIR}")
    print(f"Local mods directory: {LOCAL_MODS_DIR}")
    print(f"Output directory: {OUTPUT_DIR}")
    print()

    indexer = ModCodeIndexer()
    cs_files, mods = indexer.collect_files()

    print(f"Found {len(mods)} mods:")
    for mod in mods:
        print(f"  - [{mod['source']}] {mod['name']}")
    print()

    indexer.index_files(cs_files)
    indexer.write_indices(OUTPUT_DIR, mods)
